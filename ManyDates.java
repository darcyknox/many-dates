...
class ManyDates {
  ...
  /*
   * Uses monthScore value to assign the 'most fitting' month column. Then uses
   * dayScore value to assign the 'most fitting' day column (using the given day
   * column), and sets the remaining column as years. Once columns are set, the
   * program checks for invalidities, then formats valid dates for output.
   */
  ...
  /*
   * Returns a value representing percentage of valid days in a column. If the
   * given month is valid, the day is only considered valid if it fits within the
   * daysInEachMonth for its month. If the month isn't valid, we assume the day is
   * valid if it's less than 31. dayScore accounts for leap years by calling the
   * leapYear mutator.
   */
  ...
  /*
   * Mutator function that changes days in February if the year is a leap year, or
   * corrects it if it's not.
   */
  ...
  public static void main(String[] args) {
    ...
    /*
     * Splits each line of input by the / separator Adds each number to an array by
     * positions - all days are in the same ArrayList - all months are in the same
     * ArrayList - all years are in the same ArrayList
     */
    while (stdin.hasNextLine()) {
      ...
      /*
       * If the pattern doesn't match, an error is printed and the input doesn't get
       * processed.
       */
      if (!match || split.length != 3) {
        ...
        if ((split[0].length() > 4 || split[0].length() == 3) || (split[1].length() > 4 || split[1].length() == 3)
            || (split[2].length() > 4 || split[2].length() == 3)) {
          System.out.println(
              line + " - INVALID: Input numbers should not be more than 4 digits long, or 3 digits in length.");
          ...
          /*
           * Column 2 comes first in monthScores array so the format stays the same as the
           * input format if there are equal monthScore/dayScore values. i.e. default case
           * is d/m/y.
           */
          ...
          /*
           * From knowing the likeliest month column, assign the days column to be the
           * likeliest days column out of the two remaining.
           */
          if (monthColIndex == 2) {
          ...
          for (int i = 0; i < numberOfLines; i++) {
            ...
            if (isValidDate) {
              ...
              /*
               * Checks if month exceeds 12, and whether the day is within the days in that
               * particular month.
               */
            ...
            String dayString;
            if (!isValidDate) {
              System.out.println(Integer.toString(days[i]) + "/" + Integer.toString(months[i]) + "/"
                  + Integer.toString(years[i]) + errorString);
            } else {
              if (days[i] < 10) {
